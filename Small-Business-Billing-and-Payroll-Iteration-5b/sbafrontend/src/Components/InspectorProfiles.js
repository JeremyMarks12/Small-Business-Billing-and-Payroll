    ...
    const fetchWorkers = async () => {
        try {
            ...
            const adminList = data.filter(worker =>
                (worker.isAdmin === true || worker.isAdmin === "true" || worker.admin === true || worker.admin === "true") &&
                worker.username !== SYSTEM_ADMIN_USERNAME
            );
    
            const inspectorList = data.filter(worker =>
                (worker.isAdmin === false || worker.isAdmin === "false" || worker.admin === false || worker.admin === "false") &&
                worker.username !== SYSTEM_ADMIN_USERNAME
            );
    
            const sortWorkers = (list) =>
